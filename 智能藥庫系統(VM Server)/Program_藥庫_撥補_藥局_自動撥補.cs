...
namespace 智能藥庫系統_VM_Server_
{
    public partial class Form1 : Form
    {
        ...
        {
            SQLUI.SQL_DataGridView.SQL_Set_Properties(this.sqL_DataGridView_藥庫_撥補_藥局_自動撥補, dBConfigClass.DB_DS01);
        ...
        {
            sub_Program_檢查自動撥補至藥局();
        }

        #region PLC_檢查自動撥補至藥局
        Task Task_檢查自動撥補至藥局;
        PLC_Device PLC_Device_檢查自動撥補至藥局 = new PLC_Device("");
        PLC_Device PLC_Device_檢查自動撥補至藥局_OK = new PLC_Device("");
        MyTimer MyTimer_檢查自動撥補至藥局_結束延遲 = new MyTimer();
        int cnt_Program_檢查自動撥補至藥局 = 65534;
        void sub_Program_檢查自動撥補至藥局()
        {
            PLC_Device_檢查自動撥補至藥局.Bool = true;
            if (cnt_Program_檢查自動撥補至藥局 == 65534)
            {
                this.MyTimer_檢查自動撥補至藥局_結束延遲.StartTickTime(10000);
                PLC_Device_檢查自動撥補至藥局.SetComment("PLC_檢查自動撥補至藥局");
                PLC_Device_檢查自動撥補至藥局_OK.SetComment("PLC_檢查自動撥補至藥局_OK");
                PLC_Device_檢查自動撥補至藥局.Bool = false;
                cnt_Program_檢查自動撥補至藥局 = 65535;
            }
            if (cnt_Program_檢查自動撥補至藥局 == 65535) cnt_Program_檢查自動撥補至藥局 = 1;
            if (cnt_Program_檢查自動撥補至藥局 == 1) cnt_Program_檢查自動撥補至藥局_檢查按下(ref cnt_Program_檢查自動撥補至藥局);
            if (cnt_Program_檢查自動撥補至藥局 == 2) cnt_Program_檢查自動撥補至藥局_初始化(ref cnt_Program_檢查自動撥補至藥局);
            if (cnt_Program_檢查自動撥補至藥局 == 3) cnt_Program_檢查自動撥補至藥局 = 65500;
            if (cnt_Program_檢查自動撥補至藥局 > 1) cnt_Program_檢查自動撥補至藥局_檢查放開(ref cnt_Program_檢查自動撥補至藥局);

            if (cnt_Program_檢查自動撥補至藥局 == 65500)
            {
                this.MyTimer_檢查自動撥補至藥局_結束延遲.TickStop();
                this.MyTimer_檢查自動撥補至藥局_結束延遲.StartTickTime(60000);
                PLC_Device_檢查自動撥補至藥局.Bool = false;
                PLC_Device_檢查自動撥補至藥局_OK.Bool = false;
                cnt_Program_檢查自動撥補至藥局 = 65535;
            }
        }
        void cnt_Program_檢查自動撥補至藥局_檢查按下(ref int cnt)
        {
            if (PLC_Device_檢查自動撥補至藥局.Bool) cnt++;
        }
        void cnt_Program_檢查自動撥補至藥局_檢查放開(ref int cnt)
        {
            if (!PLC_Device_檢查自動撥補至藥局.Bool) cnt = 65500;
        }
        void cnt_Program_檢查自動撥補至藥局_初始化(ref int cnt)
        {
            if (this.MyTimer_檢查自動撥補至藥局_結束延遲.IsTimeOut())
            {
                ...
                list_過帳狀態 = list_過帳狀態.GetRows((int)enum_過帳狀態列表.檔名, "自動撥補至藥局");
                if (list_過帳狀態.Count > 0)
                {
                    if (Task_檢查自動撥補至藥局 == null)
                    {
                        Task_檢查自動撥補至藥局 = new Task(new Action(delegate 
                        {
                            PlC_RJ_Button_藥庫_撥補_藥局_自動撥補_產出今日撥補表_MouseDownEvent(null);
                            ...
                            Function_藥庫_撥補_藥局_開始撥補(list_自動撥補);
                        }));
                    }
                    if (Task_檢查自動撥補至藥局.Status == TaskStatus.RanToCompletion)
                    {
                        Task_檢查自動撥補至藥局 = new Task(new Action(delegate 
                        {
                            PlC_RJ_Button_藥庫_撥補_藥局_自動撥補_產出今日撥補表_MouseDownEvent(null);
                    ...
                    if (Task_檢查自動撥補至藥局.Status == TaskStatus.Created)
                    {
                        Task_檢查自動撥補至藥局.Start();
                    }
        ...
        private void SqL_DataGridView_藥庫_撥補_藥局_自動撥補_DataGridRefreshEvent()
        {
            ...
            for (int i = 0; i < this.sqL_DataGridView_藥庫_撥補_藥局_自動撥補.dataGridView.Rows.Count; i++)
            {
                ...
                {
                    this.sqL_DataGridView_藥庫_撥補_藥局_自動撥補.dataGridView.Rows[i].DefaultCellStyle.BackColor = Color.Lime;
        ...
        private void PlC_RJ_Button_藥庫_撥補_藥局_自動撥補_產出今日撥補表_MouseDownEvent(MouseEventArgs mevent)
        {
            ...
            for (int i = 0; i < list_藥品資料.Count; i++)
            {
                ...
                藥品碼 = list_藥品資料[i][(int)enum_medPharmacy.藥品碼].ObjectToString();
                藥品名稱 = list_藥品資料[i][(int)enum_medPharmacy.藥品名稱].ObjectToString();
                List<DeviceBasic> deviceBasic_buf = deviceBasics.SortByCode(藥品碼);
                if (deviceBasic_buf.Count == 0) continue;
                安全量 = list_藥品資料[i][(int)enum_medPharmacy.安全庫存].ObjectToString().StringToInt32();
                基準量 = list_藥品資料[i][(int)enum_medPharmacy.基準量].ObjectToString().StringToInt32();
                包裝數量 = list_藥庫_藥品資料[i][(int)enum_medDrugstore.包裝數量].ObjectToString().StringToInt32();

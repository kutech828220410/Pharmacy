...
namespace ConsoleApp_口服藥_每日自動撥補單建立
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                for (int i = 0; i < medClasses_藥局.Count; i++)
                {
                    ...
                    實撥量_口服藥_口一 = (int)((實撥量 / 5.5) * 2.6);

                    實撥量_口服藥_口二 = (int)((實撥量 / 5.5) * 2.6);

                    實撥量_口服藥_UD = (int)((實撥量 / 5.5) * 0.3);

                    撥發量_口服藥_UD = 實撥量_口服藥_UD;
                    撥發量_口服藥_口一 = 實撥量_口服藥_口一;
                    撥發量_口服藥_口二 = 實撥量_口服藥_口二;
                    ...
                    for (int k = 0; k < 3; k++)
                    {
                        ...
                        if (drugStotreDistributionClasses_buf.Count > 0)
                        {
                            ...
                            Logger.Log($"[修改]({i.ToString("00")}/{medClasses_藥局.Count}) {temp_str} 撥發量 : {撥發量} , 實撥量 : {實撥量_temp}");

                        }
                        else
                        {
                            drugStotreDistributionClasses_add.Add(drugStotreDistributionClass);
                            Logger.Log($"[新增]({i.ToString("00")}/{medClasses_藥局.Count}) {temp_str} 撥發量 : {撥發量} , 實撥量 : {實撥量_temp}");
                        }
                    }

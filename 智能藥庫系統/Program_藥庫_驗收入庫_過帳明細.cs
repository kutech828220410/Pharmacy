...
namespace 智能藥庫系統
{
    ...
    {
        public enum enum_藥庫_驗收入庫_過帳明細_驗收入庫明細
        {
            GUID,
            藥品碼,
            藥品名稱,
        ...
        public enum enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態
        {
            等待過帳,
            庫存不足,
            未建立儲位,
        ...
        {           
            ...
            {
                object[] value = list_補給驗收入庫[i].CopyRow(new enum_補給驗收入庫(),new enum_藥庫_驗收入庫_過帳明細_驗收入庫明細());

                string 藥品碼 = value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.藥品碼].ObjectToString();
                藥品碼 = 藥品碼.Substring(藥品碼.Length - 5, 5);
                value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.藥品碼] = 藥品碼;

                list_藥品資料_buf = list_藥品資料.GetRows((int)enum_藥庫_藥品資料.藥品碼, 藥品碼);
                if (list_藥品資料_buf.Count == 0) continue;

                value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.藥品名稱] = list_藥品資料_buf[0][(int)enum_藥庫_藥品資料.藥品名稱];
                value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.包裝單位] = list_藥品資料_buf[0][(int)enum_藥庫_藥品資料.包裝單位];
        ...
        private void sqL_DataGridView_藥庫_過帳明細_驗收入庫明細_DataGridRefreshEvent()
        {
            ...
            {
                狀態 = this.sqL_DataGridView_藥庫_過帳明細_驗收入庫明細.dataGridView.Rows[i].Cells[enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態.GetEnumName()].Value.ToString();
                if (狀態 == enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.過帳完成.GetEnumName())
                {
        ...
        private void PlC_RJ_Button_藥庫_驗收入庫_過帳明細_選取資料刪除_MouseDownEvent(MouseEventArgs mevent)
        {
            ...
            List<object[]> list_驗收入庫明細_buf = list_驗收入庫明細.CopyRows(new enum_藥庫_驗收入庫_過帳明細_驗收入庫明細(), new enum_補給驗收入庫());
        ...
        private void PlC_RJ_Button_藥庫_驗收入庫_過帳明細_選取資料過帳_MouseDownEvent(MouseEventArgs mevent)
        {
            ...
            {
                if (list_驗收入庫明細[i][(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態].ObjectToString() != enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.等待過帳.GetEnumName()) continue;
                string 藥品碼 = list_驗收入庫明細[i][(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.藥品碼].ObjectToString();
                string 效期 = list_驗收入庫明細[i][(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.效期].ObjectToString();
                string 批號 = list_驗收入庫明細[i][(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.批號].ObjectToString();
                string 數量 = list_驗收入庫明細[i][(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.數量].ObjectToString();

                object[] value = list_驗收入庫明細[i].CopyRow(new enum_藥庫_驗收入庫_過帳明細_驗收入庫明細(), new enum_補給驗收入庫());
                value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.藥品碼] = $"A0000{藥品碼}";
                ...
                if (deviceBasics_buf.Count > 0)
                {
                    ...
                    value[(int)enum_補給驗收入庫.狀態] = enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.過帳完成.GetEnumName();
                    list_驗收入庫明細[i][(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態] = enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.過帳完成.GetEnumName();
                    ...
                    string 藥品名稱 = list_驗收入庫明細[i][(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.藥品名稱].ObjectToString();
                    string 交易量 = 數量;
                ...
                {
                    value[(int)enum_補給驗收入庫.狀態] = enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.未建立儲位.GetEnumName();
                    list_驗收入庫明細[i][(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態] = enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.未建立儲位.GetEnumName();
                    list_驗收入庫明細_replace.Add(list_驗收入庫明細[i]);
                }
        ...
        private void PlC_RJ_Button_藥庫_驗收入庫_過帳明細_選取資料設定未過帳_MouseDownEvent(MouseEventArgs mevent)
        {
            ...
            for (int i = 0; i < list_驗收入庫明細.Count; i++)
            {
                ...
                list_驗收入庫明細[i][(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態] = enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.等待過帳.GetEnumName();
                list_驗收入庫明細_replace.Add(list_驗收入庫明細[i]);
            }
            this.sqL_DataGridView_補給驗收入庫.SQL_ReplaceExtra(list_補給驗收入庫_replace, false);
            this.sqL_DataGridView_藥庫_過帳明細_驗收入庫明細.ReplaceExtra(list_驗收入庫明細_replace, true);
        ...
        private void PlC_RJ_Button_藥庫_驗收入庫_過帳明細_選取資料設定忽略未過帳_MouseDownEvent(MouseEventArgs mevent)
        {
            ...
            for (int i = 0; i < list_驗收入庫明細.Count; i++)
            {
                ...
                list_驗收入庫明細[i][(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態] = enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.忽略過帳.GetEnumName();
                list_驗收入庫明細_replace.Add(list_驗收入庫明細[i]);
            }
            this.sqL_DataGridView_補給驗收入庫.SQL_ReplaceExtra(list_補給驗收入庫_replace, false);
            this.sqL_DataGridView_藥庫_過帳明細_驗收入庫明細.ReplaceExtra(list_驗收入庫明細_replace, true);
        ...
        private void PlC_RJ_Button_藥庫_驗收入庫_過帳明細_顯示異常過帳_MouseDownEvent(MouseEventArgs mevent)
        {
            ...
            list_value = (from value in list_value
                          where value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態].ObjectToString() == enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.庫存不足.GetEnumName()
                          || value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態].ObjectToString() == enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.找無此藥品.GetEnumName()
                          || value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態].ObjectToString() == enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.未建立儲位.GetEnumName()
                          || value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態].ObjectToString() == enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.無效期可入帳.GetEnumName()
                          || value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態].ObjectToString() == enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.庫存不足.GetEnumName()
                          select value).ToList();
        ...
        private void PlC_RJ_Button_藥庫_驗收入庫_過帳明細_顯示等待過帳_MouseDownEvent(MouseEventArgs mevent)
        {
            List<object[]> list_value = this.Function_藥庫_驗收入庫_過帳明細_取得資料();
            list_value = (from value in list_value
                          where value[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.狀態].ObjectToString() == enum_藥庫_驗收入庫_過帳明細_驗收入庫明細_狀態.等待過帳.GetEnumName()
                          select value).ToList();
            this.sqL_DataGridView_藥庫_過帳明細_驗收入庫明細.RefreshGrid(list_value);
        }
        private void PlC_RJ_Button_藥庫_驗收入庫_過帳明細_藥品名稱篩選_MouseDownEvent(MouseEventArgs mevent)
        {
            ...
            list_value = list_value.GetRowsByLike((int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.藥品名稱, this.rJ_TextBox_藥庫_驗收入庫_過帳明細_藥品名稱篩選.Text);
            this.sqL_DataGridView_藥庫_過帳明細_驗收入庫明細.RefreshGrid(list_value);
        }
        private void PlC_RJ_Button_藥庫_驗收入庫_過帳明細_藥品碼篩選_MouseDownEvent(MouseEventArgs mevent)
        {
            List<object[]> list_value = this.sqL_DataGridView_藥庫_過帳明細_驗收入庫明細.GetAllRows();
            list_value = list_value.GetRowsByLike((int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.藥品碼, this.rJ_TextBox_藥庫_驗收入庫_過帳明細_藥品碼篩選.Text);
            this.sqL_DataGridView_藥庫_過帳明細_驗收入庫明細.RefreshGrid(list_value);
        }
        #endregion
        private class ICP_驗收入庫_過帳明細 : IComparer<object[]>
        {
            ...
            {
                DateTime temp0 = x[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.驗收時間].ToDateString().StringToDateTime();
                DateTime temp1 = y[(int)enum_藥庫_驗收入庫_過帳明細_驗收入庫明細.驗收時間].ToDateString().StringToDateTime();

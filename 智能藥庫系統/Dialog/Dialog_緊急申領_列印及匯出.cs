...
namespace 智能藥庫系統
{
    public partial class Dialog_緊急申領_列印及匯出 : Form
    {
        ...
        public Dialog_緊急申領_列印及匯出(SQL_DataGridView sQL_DataGridView, string emg_apply_ApiURL)
        {
            InitializeComponent();
        ...
        private void Dialog_緊急申領_列印及匯出_Load(object sender, EventArgs e)
        {
        ...
        private List<SheetClass> GetSheetClasses(bool 設定已列印)
        {
            ...
            if (設定已列印)
            {
                for (int i = 0; i < list_value.Count; i++)
                {
                    if (list_value[i][(int)enum_藥庫_撥補_藥局_緊急申領.狀態].ObjectToString() == enum_藥庫_撥補_藥局_緊急申領_狀態.過帳完成.GetEnumName()) continue;
